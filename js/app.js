...
/* ==========================================================
   2. CONFIGURATION GITHUB & HELPERS API DISTANTE
   ========================================================== */
if (typeof getEmbeddedToken === 'undefined') {
...
if (typeof ghGetFile === 'undefined') {
  async function ghGetFile(filePath) {
    if (!ghConfig || !ghConfig.token) return null;
    ...
    const content = typeof b64ToUtf8 === 'function'
      ? b64ToUtf8(json.content)
      : decodeURIComponent(Array.prototype.map.call(atob((json.content || '').replace(/[\r\n\s]/g, '')), c => '%' + ('00' + c.charCodeAt(0).toString(16)).slice(-2)).join(''));
    return { sha: json.sha, data: JSON.parse(content) };
  }
}

if (typeof ghPutFile === 'undefined') {
  async function ghPutFile(filePath, dataObj, commitMsg) {
    ...
    const b64Content = typeof utf8ToB64 === 'function'
      ? utf8ToB64(serialized)
      : btoa(encodeURIComponent(serialized).replace(/%([0-9A-F]{2})/g, (match, p1) => String.fromCharCode('0x' + p1)));
